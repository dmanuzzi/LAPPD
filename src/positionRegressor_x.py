...
random_forest = RandomForestRegressor(verbose=1, n_jobs=n_jobs,
...
# def rf_apply(XX, model):
#     return model.apply(XX)
...
for a,b,c,d,f,g, in zip(y_hat_train,  y_train, X_train[:,-1],  X_train[:,-2],  X_train[:,-3],  X_train[:,-4]):
  outFile.write('%lf %lf %lf %lf %lf %lf\n'%(a,b,c,d,f,g))
outFile.close()

outFile = open(outDir+'/output_eval_%s.dat'%(args.outFile),'w')
for a,b,c,d,f,g, in zip(y_hat_val, y_val, X_val[:,-1],  X_val[:,-2],  X_val[:,-3],  X_val[:,-4]):
  outFile.write('%lf %lf %lf %lf %lf %lf\n'%(a,b,c,d,f,g))
outFile.close()
